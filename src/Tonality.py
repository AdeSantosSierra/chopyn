...
class Tonality (object):
	...
	def __init__(self, Tonic):
		...
		self.grades = self.tonalities[index_Tonic:]+self.tonalities[:index_Tonic]
...
class Major (Tonality):
	...
	def create_music_from_grades_sequences(self, musical_sequence, number_of_notes_per_compass):
		...
		for grade in musical_sequence:
			...
			main_note_from_grade = (globals()[self.grades[self.dict_grades_positions[grade]].__name__]())
			...
			melody_sequence.extend(np.random.choice(notes_from_chord, number_of_notes_per_compass, replace = True))
		...
		for note in melody_sequence:
			note.update_props({'duration':np.random.choice([125,250],1,replace=True)[0],'intensity':70,'timbre':1})
